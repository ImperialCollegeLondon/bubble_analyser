...
class MaskToEllipseConverter:
    ...
    def convert_mask_to_ellipses(
        self, labeled_image: npt.NDArray[np.int_], background_label: int = 1
            ...
            for contour in contours:
                if len(contour) >= self.min_contour_points:
                    try:
                        ...
                        preprocessed_contour = self._preprocess_contour(contour)

                        # Additional check after preprocessing
                        if preprocessed_contour is None or len(preprocessed_contour) < self.min_contour_points:
                            logging.warning(f"Contour preprocessing failed for label {label}")
                            continue

                        # Fit ellipse to contour
                        ellipse = cast(
                            tuple[tuple[float, float], tuple[float, float], float], cv2.fitEllipse(preprocessed_contour)
                        )
    ...
    def _preprocess_contour(self, contour: npt.NDArray[Any]) -> npt.NDArray[np.int32] | None:  # type: ignore[explicit-any]
        ...
        try:
            ...
            points: npt.NDArray[Any]  # type: ignore[explicit-any]
            if contour.ndim == 3:
            ...
            processed_contour = points.reshape(-1, 1, 2)

            # Ensure contour points are integers (required by OpenCV)
            processed_contour = processed_contour.astype(np.int32)

            # Remove duplicate consecutive points
            if len(processed_contour) > 1:
                # Calculate differences between consecutive points
                diff = np.diff(processed_contour.reshape(-1, 2), axis=0)
                ...
                processed_contour = processed_contour[keep_mask]

            # Final validation
            if len(processed_contour) < 5:  # cv2.fitEllipse needs at least 5 points
                return None
    ...
    def convert_file_to_ellipses(
        self, image_path: str, background_label: int = 1
    ) -> list[tuple[tuple[float, float], tuple[float, float], float]]:
    ...
    def get_ellipse_properties(
        ...
        for i, ellipse in enumerate(ellipses):
            ...
            np.pi * (
                3 * (major_axis + minor_axis) - np.sqrt((3 * major_axis + minor_axis) * (major_axis + 3 * minor_axis))
            )
    ...
    def process_folder(
        ...
        if not mask_files:
            raise ValueError(f"No files with '_mask.png' suffix found in {input_dir}")
        ...
        for mask_file in mask_files:
            try:
                ...
                if save_results:
                    self._save_ellipse_results(mask_file, ellipses, final_output_dir)
        ...
        try:
            ...
            if ellipses:
                ...
                ellipses_array = np.array(
                    [[ellipse[0][0], ellipse[0][1], ellipse[1][0], ellipse[1][1], ellipse[2]] for ellipse in ellipses]
                )
...
def load_ellipses_from_mask(
    image_path: str, background_label: int = 1, min_contour_points: int = 5, validate_ellipses: bool = True
) -> list[tuple[tuple[float, float], tuple[float, float], float]]:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if is_folder_mode:
            ...
            results = converter.process_folder(input_path, save_results=True, output_folder=output_folder)
