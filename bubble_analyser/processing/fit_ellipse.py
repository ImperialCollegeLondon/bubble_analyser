...
class EllipseAdjuster(QMainWindow):
    ...
    def __init__(self, ellipse_list, img_rgb):
        super().__init__()
        ...
        self.ellipses = ellipse_list.copy()
        self.B = img_rgb.copy()
        ...
        self.initial_handle_angle = None  # Angle between center and mouse when rotation started (radians).
        self.initial_ellipse_angle = None  # Ellipse's original angle at start of rotation (degrees).
    ...
    def update_image(self):
        ...
        q_img = QImage(scaled_image.data, new_size[0], new_size[1], bytes_per_line, QImage.Format_BGR888)
        self.image_label.setPixmap(QPixmap.fromImage(q_img))

    def draw_ellipses(self, image, ellipses):
        for idx, ellipse in enumerate(ellipses):
            ...
            major_axis1 = (int(center[0] + axes[0] / 2 * cos_angle), int(center[1] + axes[0] / 2 * sin_angle))
            major_axis2 = (int(center[0] - axes[0] / 2 * cos_angle), int(center[1] - axes[0] / 2 * sin_angle))
            minor_axis1 = (int(center[0] + axes[1] / 2 * -sin_angle), int(center[1] + axes[1] / 2 * cos_angle))
            minor_axis2 = (int(center[0] - axes[1] / 2 * -sin_angle), int(center[1] - axes[1] / 2 * cos_angle))
            ...
            cross_size = 7
            cv2.line(image, (center_int[0] - cross_size, center_int[1]),
                           (center_int[0] + cross_size, center_int[1]), (255, 255, 0), 2)
            cv2.line(image, (center_int[0], center_int[1] - cross_size),
                           (center_int[0], center_int[1] + cross_size), (255, 255, 0), 2)
    ...
    def add_circle_button_clicked(self):
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        for i, ellipse in enumerate(self.ellipses):
            ...
            control_points = [major_axis1, major_axis2, minor_axis1, minor_axis2, (int(center[0]), int(center[1]))] + rot_handles
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if self.moving_center:
            ...
            if self.selected_axis == 0:  # Major axis endpoint 1
                new_axes = (max(1, axes[0] + (dx * cos_angle + dy * sin_angle)), axes[1])
            elif self.selected_axis == 1:  # Major axis endpoint 2
                new_axes = (max(1, axes[0] - (dx * cos_angle + dy * sin_angle)), axes[1])
            elif self.selected_axis == 2:  # Minor axis endpoint 1
                new_axes = (axes[0], max(1, axes[1] + (-dx * sin_angle + dy * cos_angle)))
    ...
    def delete_selected_ellipse(self):
        if self.selected_ellipse_index is not None:
            del self.ellipses[self.selected_ellipse_index]
            self.selected_ellipse_index = None
            self.update_image()

    def closeEvent(self, event):
        # Create a copy of the original image and overlay the ellipses.
        display_image = self.B.copy()
        self.draw_ellipses(display_image, self.ellipses)
