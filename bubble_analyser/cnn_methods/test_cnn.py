...
def convert_bubmask_to_watershed_labels(
    masks: npt.NDArray[np.bool_] | Any, target_shape: tuple[int, int]
...
def save_watershed_labels(labels: npt.NDArray[np.int32], output_path: str | Path, image_name: str) -> None:
...
def get_image_shape(image_path: str | Path) -> tuple[int, int]:
...
def test_single_image_progressive() -> None:
    ...
    try:
        results = detector.detect_bubbles_progressive(test_image, return_masks=True, return_splash=False)
        ...
        if "masks" in results and results["masks"] is not None:
            ...
            watershed_labels = convert_bubmask_to_watershed_labels(results["masks"], original_shape)
...
def test_different_configurations() -> None:
    ...
    for name, config in configs.items():
        try:
            BubMaskDetector(weights_path, config)
...
def test_batch_detection() -> None:
    ...
    try:
        ...
        results_list = detector.batch_detect(
            input_dir=input_path, output_dir=output_path, save_masks=True, save_splash=True
        )

        # Process results and convert to watershed format
        total_bubbles = sum(int(cast(float, result["bubble_count"])) for result in results_list)
        print("\n✓ Batch Detection Complete!")
        print("  Configuration: Medium Quality (256x512)")
        ...
        for result in results_list:
            image_name = str(result.get("image_name", result.get("filename", "unknown")))
            bubble_count = int(cast(float, result.get("bubble_count", 0)))
            print(f"  Processing {image_name}: {bubble_count} bubbles")

            if "masks" in result and result["masks"] is not None:
                # Find original image to get its shape
                image_files = list(input_dir.glob(f"*{Path(image_name).stem}*"))
                if image_files:
                    original_image = image_files[0]
                    original_shape = get_image_shape(original_image)

                    # Convert to watershed labels
                    watershed_labels = convert_bubmask_to_watershed_labels(result["masks"], original_shape)

                    # Save watershed labels
                    save_watershed_labels(watershed_labels, watershed_output_path, image_name)
                else:
                    print(f"    Warning: Could not find original image for {image_name}")
            else:
                print(f"    No masks available for {image_name}")
...
def test_watershed_conversion_only() -> None:
    ...
    try:
        ...
        results = detector.detect_bubbles(test_image, return_masks=True, return_splash=False)

        if "masks" in results and results["masks"] is not None:
            original_shape = get_image_shape(test_image)

            print(f"  Original BubMask format: {results['masks'].shape}")  # type: ignore
