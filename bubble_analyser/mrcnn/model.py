...
def compute_backbone_shapes(
    config: Config, image_shape: tuple[int, int] | npt.NDArray[np.float32]
    ...
    return np.array(
        [[math.ceil(image_shape[0] / stride), math.ceil(image_shape[1] / stride)] for stride in config.BACKBONE_STRIDES]
    )
...
class ProposalLayer(KL.Layer):
    ...
    def __init__(
        self, proposal_count: int, nms_threshold: float, config: Config | None = None, **kwargs: object
    ) -> None:
    ...
    def call(self, inputs: list[tf.Tensor]) -> tf.Tensor:
        ...
        # Non-max suppression
        def nms(boxes: tf.Tensor, scores: tf.Tensor) -> tf.Tensor:
    ...
    def compute_output_shape(self, input_shape: list[tuple[int | None, ...]]) -> tuple[int | None, int, int]:
...
def log2_graph(x: tf.Tensor) -> tf.Tensor:
...
class PyramidROIAlign(KL.Layer):
    ...
    def __init__(self, pool_shape: list[int] | tuple[int, int], **kwargs: object) -> None:
        super().__init__(**kwargs)
    ...
    def compute_output_shape(self, input_shape: list[tuple[int | None, ...]]) -> tuple[int | None, ...]:
...
class DetectionTargetLayer(KL.Layer):
    ...
    def compute_output_shape(self, input_shape: list[tuple[int | None, ...]]) -> list[tuple[int | None, ...]]:
        return [
    ...
    def compute_mask(self, inputs: list[tf.Tensor], mask: tf.Tensor | None = None) -> list[None]:
...
def refine_detections_graph(
    ...
    def nms_keep_map(class_id: tf.Tensor) -> tf.Tensor:
...
def rpn_graph(feature_map: tf.Tensor, anchors_per_location: int, anchor_stride: int) -> list[tf.Tensor]:
    ...
    rpn_bbox = KL.Lambda(
        lambda t: tf.reshape(t, [tf.shape(t)[0], -1, 4]), output_shape=lambda input_shape: (input_shape[0], None, 4)
    )(x)
...
def build_fpn_mask_graph(
    ...
    for i in range(1, num_deconv_layers):
        x = KL.TimeDistributed(
            KL.Conv2DTranspose(256, (2, 2), strides=2, activation="relu"), name="mrcnn_mask_deconv" + str(i + 1)
        )(x)
...
def weight_cus(y_true: tf.Tensor, config: Config) -> tf.Tensor:
    ...
    weight = (weight - config.MEAN_SIZE / config.MAX_SIZE) / (
        config.MEAN_SIZE / config.MIN_SIZE - config.MEAN_SIZE / config.MAX_SIZE
    ) - 0.5
...
def rpn_class_loss_graph(rpn_match: tf.Tensor, rpn_class_logits: tf.Tensor) -> tf.Tensor:
...
def rpn_bbox_loss_graph(config: Config, target_bbox: tf.Tensor, rpn_match: tf.Tensor, rpn_bbox: tf.Tensor) -> tf.Tensor:
...
def mrcnn_class_loss_graph(
    target_class_ids: tf.Tensor, pred_class_logits: tf.Tensor, active_class_ids: tf.Tensor
) -> tf.Tensor:
...
def mrcnn_bbox_loss_graph(
    config: Config, target_bbox: tf.Tensor, target_class_ids: tf.Tensor, pred_bbox: tf.Tensor
) -> tf.Tensor:
...
def mrcnn_mask_loss_graph(target_masks: tf.Tensor, target_class_ids: tf.Tensor, pred_masks: tf.Tensor) -> tf.Tensor:
...
def load_image_gt(
    ...
    if augmentation:
        ...
        def hook(images: npt.NDArray[np.float32], augmenter: object, parents: list[object], default: object) -> bool:
...
class MaskRCNN:
    ...
    def find_last(self) -> str:
        ...
        if not dir_names:
            ...
            raise FileNotFoundError(errno.ENOENT, f"Could not find model directory under {self.model_dir}")
        # Pick last directory
    ...
    def load_weights(self, filepath: str, by_name: bool = False, exclude: list[str] | None = None) -> None:
        ...
        # Use modern TensorFlow/Keras load_weights method
        if exclude:
            by_name = True

        # In multi-GPU training, we wrap the model. Get the actual model
        keras_model = self.keras_model
        if hasattr(keras_model, "inner_model"):
            keras_model = keras_model.inner_model

        # Use the built-in load_weights method
    ...
    def compile(self, learning_rate: float, beta_1: float, beta_2: float) -> None:
        ...
        optimizer = keras.optimizers.Adam(
            learning_rate=learning_rate, beta_1=beta_1, beta_2=beta_2, clipnorm=self.config.GRADIENT_CLIP_NORM
        )
        # Add Losses
        ...
        self.keras_model.compile(
            optimizer=optimizer, loss=[None] * len(self.keras_model.outputs), metrics=["accuracy", "calculate_mAP"]
        )
    ...
    def set_log_dir(self, model_path: str | None = None) -> None:
        ...
        self.checkpoint_path = os.path.join(self.log_dir, f"mask_rcnn_{self.config.NAME.lower()}_*epoch*.h5")
    ...
    def train(
        ...
        layers: str,
        augmentation: Any | None = None,
        custom_callbacks: list[Callback] | None = None,
        no_augmentation_sources: list[str] | None = None,
    ) -> None:
        """Train the model.
        ...
        callbacks = [
            ...
            keras.callbacks.ModelCheckpoint(
                self.checkpoint_path, verbose=0, save_weights_only=True, mode="auto", period=1
            ),
        ]
    ...
    def mold_inputs(
        ...
        for image in images:
            ...
            molded_image, window, scale, _padding, _crop = utils.resize_image(
                image,
        ...
        # Pack into arrays
        molded_images_arr = np.stack(molded_images)
        image_metas_arr = np.stack(image_metas)
        windows_arr = np.stack(windows)
        return molded_images_arr, image_metas_arr, windows_arr

    def unmold_detections(
        self,
        detections: npt.NDArray[np.float32],
        mrcnn_mask: npt.NDArray[np.float32],
        ...
        full_masks_arr = np.stack(full_masks, axis=-1) if full_masks else np.empty((*original_image_shape[:2], 0))

        return boxes, class_ids, scores, full_masks_arr

    def detect(
        self, images: list[npt.NDArray[np.float32]], verbose: int = 0
    ...
    def detect_molded(
        ...
        for i, image in enumerate(molded_images):
            window = np.array([0, 0, image.shape[0], image.shape[1]])
            final_rois, final_class_ids, final_scores, final_masks = self.unmold_detections(
    ...
    def get_anchors(self, image_shape: tuple[int, ...]) -> npt.NDArray[np.float32]:
    ...
    def get_trainable_layers(self) -> list[KL.Layer]:
    ...
    def run_graph(
        ...
        kf = K.function(model.inputs, list(outputs_dict.values()))
        ...
        outputs_np_dict = OrderedDict([(k, v) for k, v in zip(outputs_dict.keys(), outputs_np)])
        for k, v in outputs_np_dict.items():
            log(k, v)
        return outputs_np_dict
...
def compose_image_meta(
    image_id: int,
    original_image_shape: tuple[int, int, int] | npt.NDArray[np.float32],
    image_shape: tuple[int, int, int] | npt.NDArray[np.float32],
    window: tuple[int, int, int, int] | npt.NDArray[np.float32],
    scale: float,
    active_class_ids: npt.NDArray[np.float32],
) -> npt.NDArray[np.float32]:
...
def unmold_image(normalized_images: npt.NDArray[np.float32], config: Config) -> npt.NDArray[np.float32]:
...
class MeanAveragePrecisionCallback(Callback):
    ...
    def on_epoch_end(self, epoch: int, logs: dict[str, float] | None = None) -> None:
        if epoch > 0 and epoch % self.calculate_at_every_X_epoch == 0:
            ...
            self._verbose_print(f"mAP(50, 75, 90) at epoch {epoch} is: {mAP_50}, {mAP_75}, {mAP_90}")
            self._verbose_print(f"mAP(small, medium, large) at epoch {epoch} is: {mAP_sm}, {mAP_me}, {mAP_la}")

        super().on_epoch_end(epoch, logs)

    def _load_weights_for_model(self) -> None:
        last_weights_path = self.train_model.find_last()
        self._verbose_print(
            f"Loaded weights for the inference model (last checkpoint of the train model): {last_weights_path}"
        )
        self.inference_model.load_weights(last_weights_path, by_name=True)

    def _calculate_mean_average_precision(self, iou_threshold: float) -> npt.NDArray[np.float32]:
        mAPs = []
        ...
        for image_id in self.dataset_image_ids[: self.dataset_limit]:
            ...
            AP, _, _, _ = utils.compute_ap(
                gt_bbox, gt_class_id, gt_mask, r["rois"], r["class_ids"], r["scores"], r["masks"], iou_threshold
            )
            mAPs.append(AP)

        return np.array(mAPs)

    def _calculate_mean_average_precision_size(self) -> npt.NDArray[np.float32]:
        mAPs = []
        ...
        for image_id in self.dataset_image_ids[: self.dataset_limit]:
            ...
            r = results[0]
            # Compute mAP
            AP = utils.compute_ap_size(
                gt_bbox, gt_class_id, gt_mask, r["rois"], r["class_ids"], r["scores"], r["masks"], verbose=0
            )
            mAPs.append(AP)
        ...
        with tf.compat.v1.Session():
            ...
            mAP = mAP.eval()
        mAP_final: npt.NDArray[np.float32] = np.nanmean(mAP, axis=0)
