...
def save_separate_outputs(image, masks, result_path, base_filename):
    ...
    if masks.shape[-1] > 0:
        for i in range(masks.shape[-1]):
            ...
            skimage.io.imsave(os.path.join(result_path, mask_filename), 
                            img_as_uint(masks[:,:,i]))
...
if __name__ == '__main__':
    ...
    parser.add_argument("command",
                        metavar="<command>",
                        help="'train' or 'splash' or 'detect' or 'batch_splash'")
    ...
    parser.add_argument('--save_separate', required=False,
                        action='store_true',
    ...
    if args.command == "train":
        ...
        print("'{}' is not recognized. "
              "Use 'train', 'splash', 'batch_splash', or 'detect'".format(args.command))
