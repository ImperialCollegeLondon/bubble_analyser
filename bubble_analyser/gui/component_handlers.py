...
import cv2
from pathlib import Path
from bubble_analyser.processing import Config, calculate_px2mm, Image, MethodsHandler, EllipseAdjuster
...
class ImageProcessingModel:
    ...
    def initialize_methods_handlers(self):
    ...
    def confirm_folder_selection(self,
                                folder_path_list: list[str]) -> None:
        self.img_path_list = folder_path_list
        
    def get_bknd_img_path(self,
                        bknd_img_path: str) -> Path:
        self.bknd_img_path = Path(bknd_img_path)
    
    def update_px2mm(self, px2mm: float):
        self.px2mm = px2mm
        
    def preview_processed_image(self, index):
    ...
    def load_segment_params(self, dict_params: dict):
        
        self.segment_param_dict = dict_params
    
    def load_filter_params(self, dict_params: dict):
        
        self.filter_param_dict = dict_params
        
    def processing_image_before_filtering(self):
    ...
    def initialize_image(self, name: str):
        if name not in self.img_dict:
            self.img_dict[name] = Image(self.px2mm, 
                                        raw_img_path=name,
                                        all_methods_n_params=self.all_methods_n_params,
                                        methods_handler=self.methods_handler,
                                        bknd_img_path = self.bknd_img_path)
        
    def step_1_main(self, index, algorithm: str = ""):
        name = self.img_path_list[index]
        self.initialize_image(name)
        
        self.img_dict[name].processing_image_before_filtering(algorithm)
        # if algorithm == "normal_watershed":
        #     self.img_dict[name].initialize_normal_watershed()
        #     self.img_dict[name].run_normal_watershed()
        
        # if algorithm == "iterative_watershed":
        #     self.img_dict[name].initialize_iterative_watershed()
        #     self.img_dict[name].run_iterative_watershed()

        return self.img_dict[name].labels_on_img_before_filter

    def step_2_main(self, index):
        name = self.img_path_list[index]
    ...
    def ellipse_manual_adjustment(self, index):
        name = self.img_path_list[index]
        image = self.img_dict[name]
        self.adjuster = EllipseAdjuster(image.ellipses, 
                                        image.img_rgb)
    ...
    def handle_ellipse_adjustment_finished(self, 
                                           image: Image):
    ...
    def batch_process_images(self, 
        ...
        for index, name in enumerate(self.img_path_list):
            ...
            if self.img_dict[name].if_fine_tuned:
                ...
                if if_save:
                    self.save_processed_images(self.img_dict[name].ellipses_on_images, 
                                               name, 
                                               save_path)
                    continue
    ...
    def save_processed_images(self, img, img_name, save_path):
