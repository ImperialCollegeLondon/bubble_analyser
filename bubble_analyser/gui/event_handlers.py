from bubble_analyser.gui import CalibrationModel, ImageProcessingModel, InputFilesModel
...
class ExportSettingsHandler(QDialog):
    def __init__(self, parent = None, params: Config = None):
        super().__init__(parent)
...
class FolderTabHandler:
    def __init__(self, file_handler: InputFilesModel,
                 params: Config):
        self.file_handler = file_handler
        # self.gui = gui
        self.image_path = params.raw_img_path
    
    def load_gui(self, gui):
        self.gui = gui
        
    def select_folder(self):
        if self.file_handler.sample_images_confirmed:
    ...
    def _update_folder_path(self, folder_path: str):
        ...
        self.file_handler.folder_path = folder_path
        self.gui.folder_path_edit.setText(folder_path)
    
    def _populate_image_list(self, folder_path: str):
        images, _ = self.file_handler.get_image_list(folder_path)
    ...
    def confirm_folder_selection(self):
        ...
        if folder_path:
            self._update_folder_path(folder_path)
            self._populate_image_list(folder_path)
            self.file_handler.confirm_folder_selection(folder_path)
            self.gui.tabs.setCurrentIndex(self.gui.tabs.indexOf(self.gui.calibration_tab))
            
    def preview_image_folder_tab(self):
...
class CalibrationTabHandler:
    def __init__(self,
        ...
        self.img_resample = params.resample
        self.px_img_path: Path = params.ruler_img_path
    
    def load_gui(self, gui):
        self.gui = gui
        
    def select_px_mm_image(self):
        if self.calibration_model.calibration_confirmed:
            self._show_warning("Selection Locked", "You have already confirmed the ruler image selection.")
            return
    ...
    def get_px2mm_ratio(self):
        ...
        if os.path.exists(img_path):
            ...
            px2mm = self.calibration_model.get_px2mm_ratio(img_resample = self.img_resample, gui=self.gui)
    ...
    def select_bg_corr_image(self):
        
        if self.calibration_model.calibration_confirmed:
            self._show_warning("Selection Locked", "You have already confirmed the background image selection.")
            return
        
        image_path, _ = QFileDialog.getOpenFileName(
            self.gui, "Select Background Image", "", "Image Files (*.png *.jpg *.jpeg *.bmp)"
        )
...
class ImageProcessingTabHandler(QThread):
    ...
    def __init__(self,
                 ...
                 params: Config,
                 export_handler: ExportSettingsHandler = None):
        super().__init__()
        self.model = image_processing_model
        self.params = params
        ...
        self.pass_segment_params(self.temp_param_dict)
        
    def load_gui(self, gui):
        self.gui = gui
        
    def pass_segment_params(self, param_dict: dict):
        self.model.load_segment_params(param_dict)
    
    def pass_filter_params(self, filter_param_dict: dict):
    ...
    def initialize_algorithm_combo(self):
        ...
        self.update_algorithm(self.gui.algorithm_combo.currentText())
    ...
    def handle_algorithm_change(self, new_algorithm: str):
        self.update_segment_parameters(self.current_algorithm)
        self.load_parameter_table_1(new_algorithm)
        self.update_model_algorithm(new_algorithm)
        
    def update_model_algorithm(self, algorithm: str):
        self.model.algorithm = algorithm
    ...
    def confirm_parameter_before_filtering(self, 
                                           algorithm: str):
        
        
        # Update the model parameters

        self.update_segment_parameters(algorithm)

        for algorithm_name, params in self.model.all_methods_n_params.items():
            if algorithm_name == algorithm:
                for row, (name, value), in enumerate(params.items()):
                    print("Checking steps in confirm_parameter_before_filtering: ", name, value)
                    if_valid = self.check_params(name, value)
                    if not if_valid:
                        return
                    
        # self.pass_segment_params(self.model.segment_param_dict)
        self._process_step_1(algorithm)
    ...
    def convert_value(self, text: str):
        try:
            value = float(text)
    ...
    def update_segment_parameters(self, algorithm: str) -> bool:
    ...
    def _process_step_1(self,
        ...
        step_1_img = self.model.step_1_main(self.current_index, algorithm)  
        self.update_label_before_filtering(step_1_img)
    
    def update_label_before_filtering(self, img):
    ...
    def initialize_parameter_table_2(self):
    ...
    def confirm_parameter_for_filtering(self):
    ...
    def update_if_save_processed_images(self, state):        
    ...
    def batch_process_images(self) -> None:
        ...
        self.worker_thread = WorkerThread(self.model,
                                          self.if_save_processed_images,
                                          self.export_handler.save_path)
...
class ResultsTabHandler:
    def __init__(self, 
        ...
        self.export_handler: ExportSettingsHandler = event_handler
        
    def load_gui(self, gui):
        self.gui = gui
        
    def load_ellipse_properties(self, properties):
    ...
    def generate_histogram(self):
        ...
        try:
            ...
            error = str(e)
            self._show_warning("Error in Histogram Generation", f'{error}, please process the images first.')
            return  
    ...
    def get_equivalent_diameters_list(self):
    ...
    #     folder_path = QFileDialog.getExistingDirectory(self.gui, "Select Folder to Save")
    #     if folder_path:
    #         self.gui.save_folder_edit.setText(folder_path)
    #     pass
...
class MainHandler:
    ...
    def initialize_handlers(self):
        ...
        self.folder_tab_handler = FolderTabHandler(self.input_file_model,
                                       params = self.toml_handler.params)
        
        self.calibration_model = CalibrationModel()
        self.calibration_tab_handler = CalibrationTabHandler(self.calibration_model,
                                       params = self.toml_handler.params)
        
        self.image_processing_model = ImageProcessingModel(params = self.toml_handler.params)
        self.image_processing_tab_handler = ImageProcessingTabHandler(self.image_processing_model,
                                       params = self.toml_handler.params)
        
        self.results_tab_handler = ResultsTabHandler(params=self.toml_handler.params)
        
    def initialize_handlers_signals(self):
    ...
    def load_gui_for_handlers(self):
    ...
    # Functions that needs more than one handler or model
    def confirm_folder_selection(self):
    ...
    def start_generate_histogram(self):
    ...
    def ask_if_restart(self):
        restart = QMessageBox.question(self.gui, "Restart", "Do you want to start a new mission? \n Current unsaved progress will be lost", QMessageBox.Yes | QMessageBox.No)
    ...
    def reinitialize_main_handler(self):
        self.__init__()
