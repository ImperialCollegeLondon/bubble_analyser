...
from bubble_analyser.cnn_methods.bubmask_wrapper import BubMaskConfig, BubMaskDetector, detect_bubbles_simple
...
def example_advanced_detection() -> None:
    ...
    try:
        ...
        if "masks" in results:
            masks = cast(npt.NDArray[np.bool_], results["masks"])
            properties = detector.get_bubble_properties(masks, pixel_to_mm=0.1)
