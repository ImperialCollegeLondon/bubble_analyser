                ...
                if contour_length >= 5:
                    try:
                        ...
                        if major_axis > 0 and minor_axis > 0 and major_axis / minor_axis < 10:
                            ...
                            valid_shapes += 1
                            logging.debug(f"Fitted ellipse for shape {label}: center={center}, axes={axes}, angle={angle}")
                        else:
                            logging.debug(f"Filtered degenerate ellipse for shape {label}: axes ratio = {major_axis/minor_axis if minor_axis > 0 else 'inf'}")
...
def process_mt_image_from_watershed(img_grey_morph_eroded: np.ndarray, 
                                   px2mm: float = 1, 
                                   resample: float = 1,
                                   min_area: int = 50,
                                   min_contour_length: int = 10) -> Tuple[List, List[Dict]]:
    """
    ...
    """
    processor = MTImageProcessor(px2mm=px2mm, resample=resample, 
                                min_area=min_area, min_contour_length=min_contour_length)
...
if __name__ == "__main__":
    ...
    if binary_image is not None:
        ...
        for prop in properties:
            print(f"Shape {prop['shape_id']}: Area = {prop['area_mm2']:.2f} mm², "
