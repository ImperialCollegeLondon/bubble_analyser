...
class PSDAnalyzer:
    ...
    def radial_psd(
        self, P2: np.ndarray, fy: np.ndarray, fx: np.ndarray, K: int = None
    ...
    def plot_radial_psd(
        self, f_centers: np.ndarray, radial_curve: np.ndarray, save_path: str, title: str = "Radial PSD Curve"
    ) -> None:
    ...
    def aggregate_group_features(self, per_image_features: list[dict[str, Any]] = None) -> dict[str, Any]:  # type: ignore
        ...
        out["n_images"] = len(per_image_features)
        return out
    ...
    def process_folder(
        self, folder: str, compute_anisotropy: bool = False, plot_path: str | None = None
    ) -> dict[str, Any]:
        ...
        for fp in files:
            ...
            d = self.compute_psd_features(
                img, compute_anisotropy=compute_anisotropy, plot_path=plot_path, image_name=image_name
            )
            feats.append(d)
...
if __name__ == "__main__":
    ...
    feat = analyzer.compute_psd_features(img, compute_anisotropy=True, plot_path=plot_output_dir, image_name=image_name)
    print(feat)
